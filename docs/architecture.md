...
The agent registry: a stable identity for one kind of asset, and the lifecycle and
version metadata that go with it.

```
POST /organizations/{id}/agents ─► AgentCreate ─► owner resolution ─► AgentRepository.register
                                        │                                 │
                                        ├─ identity_metadata validated    ├─ asset staged (or adopted)
                                        └─ category/version validated     └─ identity generated by PostgreSQL
                                                                               in the same transaction
```

Three decisions shape it, and each one is a boundary the phase refuses to cross:

**An agent is an asset, with identity on top.** The registry does not introduce a
second agent concept: an agent *is* the Phase 3 `agent` asset, and `agents` is a
one-to-one extension of it — `asset_id` is unique, `(organization_id, asset_id)`
references the inventory, and deleting the record cascades the identity away. The
display name, lifecycle state, environment and owner stay on the asset, where Phase
3 already constrains them; the registry adds `identity_id`, `category`, `version`,
`build_revision`, `framework` and `identity_metadata`. That is why a rename or a
version bump cannot move an identity, and why there is no second copy of anything
to keep in sync.

**Identity is generated by the database, and it is not a credential.** The column
default is `gen_random_uuid()` and no write path sets it, so no request can choose
or spoof an agent's identity — the request models have no field for it at all. It
is unique per organization (the tenant is the security boundary, and a per-tenant
lookup is what a per-tenant registry needs) and it authenticates nothing: it names
an agent for later phases, it does not authorize one. No secret material is stored,
not even hashed.

**Registering is not running.** `draft`, `active`, `suspended` and `retired` are
recorded states with a closed transition table; a registration may not start an
agent suspended or retired, moving an agent is a decision that is recorded, and
`retired` is terminal. Nothing in this phase starts, stops, contains or watches an
agent, and no permission claims to: there is no `agent.execute`, `agent.suspend`,
`agent.control` or `agent.policy.*`, and a test asserts the agent permission
